...
const match40Points = { "Priyansh Arya": 71 };
...
for (let i = 0; i < headers.length; i++) {
  if (headers[i] === "Jenna Morrh Warriors" && labels[i] === "Player Name") {
    let pointsIdx = -1;
    ...
    const roles = teamRoles["Jenna Morrh Warriors"];
    ...
    console.log(`Jenna Match 39 Correct Total: ${calculatedTotal}`);
  }
}
