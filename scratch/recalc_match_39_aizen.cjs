...
const match40Points = { "Vaibhav Sooryavanshi": 101 }; // Only player from Aizen's team who played Match 40
...
for (let i = 0; i < headers.length; i++) {
  if (headers[i] === "Aizen" && labels[i] === "Player Name") {
    ...
    for (let r = 2; r < rows.length; r++) {
      ...
      // Subtract Match 40 points to get Match 39 state
    ...
    console.log(`Aizen Match 39 Correct Total: ${calculatedTotal}`);
  }
}
