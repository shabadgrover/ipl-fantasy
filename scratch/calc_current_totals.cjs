...
discoveredTeams.forEach(base => {
  ...
  for (let i = 2; i < rows.length; i++) {
    ...
    const points = parseFloat(row[base.pointsCol]) || 0;
    
    total += points * multiplier;
  }
  
  // MOM bonuses for C/VC are NOT in the sheet points usually, they are added separately
  // Wait, let's check if the sheet points already include them.
  // Actually, usually I add them manually in the calculation script.
  // Let's see if Shreyas Iyer (Maat maro C) got a bonus.
  
  results[base.name] = total;
});
